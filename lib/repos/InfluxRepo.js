...
class InfluxRepo {
	...
	addHttpMetric({ statusCode, path, duration, userId, reqId, method, userAgent, roles, ip }) {
		...
		if (this.ipLookupEnabled && this.ipLookupDb && ip) {
			...
			if (lookedUpLocation && lookedUpLocation.location) {
				locationFields = {
					...
					country: lookedUpLocation.country ? lookedUpLocation.country.iso_code : "",
					city: lookedUpLocation.city ? lookedUpLocation.city.names.en : "",
				}
			}
		}
		...
		this.influx.addPoint({
			...
			timestamp: new Date(),
			tags: { statusCode, geohash: locationFields.geoHash }
		});
	}
}
