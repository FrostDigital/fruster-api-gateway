...
class FrusterWebBus {
    ...
    async  _newConnection(socket) {
        ...
        try {
            ...
            if (this._validatePermissions(loggedInUser)) {
                ...
                this._addCloseHandlerForConnection(socket, loggedInUser, connectionId);
                this._addMessageHandlerForConnection(socket, loggedInUser);
    ...
    _addCloseHandlerForConnection(socket, loggedInUser, connectionId) {
    ...
    _addMessageHandlerForConnection(socket, loggedInUser) {
        socket.on("message", async (msg) => {
            ...
            const responseForWebsocket = await bus.request({
                ...
                message: {
                    headers: headers,
                    reqId: message.message.reqId,
                    data: Object.assign({}, message.message.data),
            ...
            console.log("=======================================");
            console.log(require("util").inspect(responseForWebsocket, null, null, true));
            console.log("\n");

            // bus.publish(`ws.${loggedInUser.id}.${message.message.reqId}.resp`, {
            //     reqId: responseForWebsocket.reqId,
            //     data: responseForWebsocket.data
            // });
