...
class FrusterWebBus {
	...
	_registerConnectionHandler() {
		this.webSocketServer.on('connection', (socket) => this._newConnection(socket));
	}
		...
		return apiGateway.decodeToken(decodeTokenRequest, reqId)
			.then(loggedInUser => {
				...
				if (this._validatePermissions(loggedInUser)) {
					...
					log.debug("client", connectionId, "for user", loggedInUser.id, "is online");
					...
					socket.on('close', (reasonCode, description) => {
						...
						log.debug("client", connectionId, "for user", loggedInUser.id, "is offline", reasonCode, description);
					});
				} else {
					this._close(socket);
				}
