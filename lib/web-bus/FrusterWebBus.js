...
class FrusterWebBus {
	...
	_handleMessagesToWebsocket(req, actualSubject) {
		...
		if (isPublish) {
			if (!req.to || req.to.length === 0) this._publishToAllClients(req, actualSubject);
			else this._sendToAllConnectionsForClients(req.to, req, actualSubject);
		} else {
	...
	async _newConnection(socket) {
		...
		try {
			let connectedUser = await this._authServiceClient.decodeToken(reqId, jwtToken);
	...
	async _handleIncomingWebsocketRequest(msg, socket) {
		...
		if (socket.jwtToken) {
			connectedUserObject = await this._authServiceClient.decodeToken(json.message.reqId, socket.jwtToken);
		}
