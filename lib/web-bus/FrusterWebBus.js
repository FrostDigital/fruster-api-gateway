...
class FrusterWebBus {
	static get endpoints() {
		return {
			...
			SEND_MESSAGE_TO_CLIENT_PUBLIC: "ws.out.public.>",
	...
	constructor(server, options) {
		...
		bus.subscribe({
			...
			docs: docs.ws.SEND_MESSAGE_TO_CLIENT,
			handle: (req, replyTo, actualSubject) => this._handleMessagesToWebsocket(req, actualSubject)
		});

		bus.subscribe({
			subject: FrusterWebBus.endpoints.SEND_MESSAGE_TO_CLIENT_PUBLIC,
			createQueueGroup: false, // No websocket endpoint should register queue groups since clients may be spread over several instances of api gateway.
			docs: docs.ws.SEND_MESSAGE_TO_CLIENT_PUBLIC,
			handle: (req, replyTo, actualSubject) => this._publishToAllClients(req, actualSubject)
		});
		...
		bus.subscribe({
			...
			createQueueGroup: false, // No websocket endpoint should register queue groups since clients may be spread over several instances of api gateway.
			docs: docs.service.UNREGISTER_CLIENT,
	...
	_handleMessagesToWebsocket(req, actualSubject) {
		...
		if (isPublic) // Since we do not have overlapping subjects yet (coming soon), we just have to check here if the `SEND_MESSAGE_TO_CLIENT_PUBLIC` endpoint is what is used.
	...
	_handleUnregister(req) {
		...
		if (!!jwtToken) _unregisterClientByJwtToken();
		else _unregisterClientByUserId();
		...
		function _unregisterClientByUserId() {
			if (that._connectedClients[userId])
				// @ts-ignore
				Object.values(that._connectedClients[userId]).forEach(client =>
					that._close(client, constants.websocketErrorCodes.USER_DISCONNECTED)
				);
		}
	}
	 ...
	 * In 99% of cases this should be (un?)necessary, but for tests it is.
	 */
	...
	async _newConnection(socket) {
		...
		const jwtToken = this._getJwtFromUpgradeRequest(socket.upgradeReq);
		...
		try {
			...
			if (jwtToken)
				connectedUser = await AuthServiceClient.decodeToken(reqId, jwtToken);
			...
			if (!this._connectedClients[connectedUser.id]) this._connectedClients[connectedUser.id] = {};
			...
			socket.on("close", (reasonCode, description) =>
				this._handleCloseEventForConnection(socket, connectedUser, connectionId, reasonCode, description)
			);
	...
	_handleCloseEventForConnection(socket, connectedUser, connectionId, reasonCode, description) {
		...
		if (_.size(this._connectedClients[connectedUser.id]) === 0) delete this._connectedClients[connectedUser.id];
	...
	async _handleIncomingWebsocketRequest(msg, socket) {
		...
		if (socket.jwtToken) {
			connectedUserObject = await AuthServiceClient.decodeToken(json.message.reqId, socket.jwtToken);
		}
		...
		if (json.message.query) req.query = json.message.query;

		if (json.message.params) req.params = json.message.params;
	...
	_validatePermissions(loggedInUser) {
		return (
			conf.webSocketPermissionScope.filter(scope => loggedInUser.scopes.includes(scope)).length ===
			conf.webSocketPermissionScope.length
		);
	}
	...
	/**
	 * Get jwt token from one either cookie, authorization header or query param
	 * named `token`.
	 *
	 * @param {*} upgradeReq
	 */
	_getJwtFromUpgradeRequest(upgradeReq) {
		let jwtToken = (upgradeReq.headers.cookie && upgradeReq.headers.cookie[conf.authCookieName])
			|| upgradeReq.headers.Authorization;

		if (jwtToken) jwtToken = jwtToken.replace("Bearer ", "");

		if (!jwtToken) {
			if (upgradeReq.url.indexOf("?token=")) {
				const [_, token] = upgradeReq.url.split("?token=");
				jwtToken = token;
			}
		}
		return jwtToken;
