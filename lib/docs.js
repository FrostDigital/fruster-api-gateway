module.exports = {
	...
	sse: {
		SEND_EVENT_TO_CLIENT: {
			description:
				"Sends an SSE event to a specific user on a specific channel. The event is sent to a userId with a channel name (in place of the `>`). If sent to `sse.out.e3bcf884-8b49-46ae-8546-8dcc65b56932.notifications` (The channel being `notifications`) the user will receive the event on their SSE connection to the `/sse/notifications` endpoint.",
			query: {},
			params: {
				userId: "The ID of the user to send the event to",
			},
			data: "The event data to send (will be JSON stringified)",
			errors: {
				INTERNAL_SERVER_ERROR: "Something unexpected happened",
			},
		},

		SEND_EVENT_TO_ALL_CLIENTS: {
			description:
				"Broadcasts an SSE event to all users subscribed to a specific channel. If sent to `sse.out.*.system-alerts` (The channel being `system-alerts`), all users connected to the `/sse/system-alerts` endpoint will receive the event.",
			query: {},
			params: {},
			data: "The event data to send (will be JSON stringified)",
			errors: {
